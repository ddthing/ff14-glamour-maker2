    ...
    ko: {
      ...
      "terms.content": `
        ...
        <section class="public-section">
          <h2>2. 이용자 콘텐츠와 권리</h2>
          <ul>
            <li>업로드하는 사진, 이미지, 문구를 사용할 권리와 필요한 동의를 이용자가 확보해야 합니다.</li>
        ...
        <section class="public-section">
          <h2>4. 금지되는 이용</h2>
          <p>법령을 위반하거나 타인의 권리를 침해하는 이용, 서비스에 과도한 부하를 주는 자동화 요청, 보안 우회·공격, 운영자를 사칭하는 행위는 허용되지 않습니다. 문제가 확인되면 관련 요청이나 접근을 제한할 수 있습니다.</p>
        </section>
        <section class="public-section">
          <h2>5. 외부 서비스와 변경</h2>
          <p>아이템 검색, 이미지 처리, 글꼴, 호스팅 및 후원 결제에는 외부 제공자가 사용될 수 있습니다. 외부 서비스의 이용 조건과 개인정보 처리 방식은 해당 제공자의 안내가 적용됩니다.</p>
          <p>약관이나 서비스가 크게 변경되면 이 페이지의 최종 수정일과 내용을 갱신합니다. 변경 후에도 서비스를 계속 사용하면 변경된 기준에 동의한 것으로 볼 수 있습니다.</p>
        </section>
      ...
      "privacy.content": `
        <section class="public-notice">
          ...
          <ul>
            ...
            <li>룩, 사진, 편집 설정은 기본적으로 사용 중인 브라우저의 로컬 저장소와 IndexedDB에 보관됩니다.</li>
            <li>배경 제거를 요청하면 이미지가 사이트의 배경 제거 API로 전송됩니다. 서버 제공자가 설정된 환경에서는 처리에 필요한 범위에서 해당 제공자로 전달될 수 있습니다.</li>
            <li>아이템 검색과 아이콘 표시에는 외부 데이터·CDN 요청이 발생할 수 있습니다.</li>
          </ul>
        </section>
        <section class="public-section">
          <h2>1. 처리하는 정보</h2>
        ...
        <section class="public-section">
          <h2>2. 브라우저 저장과 삭제</h2>
          <p>계정 서버에 룩을 저장하는 방식이 아니라, 현재 브라우저에서 다음 방문을 돕기 위해 로컬 저장 기능을 사용합니다. 브라우저의 사이트 데이터 삭제, 다른 브라우저 사용, 또는 편집기의 전체 로컬 데이터 삭제를 하면 저장된 룩과 이미지가 사라질 수 있습니다.</p>
          <p>브라우저에서 삭제해도 외부 제공자의 네트워크·보안 로그까지 즉시 지워진다고 보장할 수는 없습니다. 외부 제공자 로그의 보관과 삭제는 해당 제공자의 정책을 따릅니다.</p>
        </section>
        <section class="public-section">
          <h2>3. 외부 제공자</h2>
          <p>사이트 호스팅과 서버 기능, 아이템 데이터와 아이콘, 선택적 글꼴 CDN, 배경 제거 처리에는 운영에 필요한 외부 제공자가 사용될 수 있습니다. 이 과정에서 요청 IP, 브라우저 정보, 요청 시각 같은 일반적인 서버 로그가 처리될 수 있습니다. 실제 보관 기간과 처리 장소는 각 제공자의 최신 정책을 확인해 주세요.</p>
          <p>현재 배포 코드에서 광고·분석 쿠키를 사용하는 기능은 확인되지 않았습니다. 향후 광고나 분석 도구를 추가하면 그 목적과 선택 방법을 이 방침에 반영하겠습니다.</p>
        </section>
        <section class="public-section">
          <h2>4. 문의와 권리 요청</h2>
          <p>개인정보 관련 질문, 삭제 요청, 권리 침해 신고는 <a class="public-inline-link" href="../contact/">문의 페이지</a>의 이메일로 보내 주세요. 요청 확인에 필요한 범위에서 추가 정보를 요청할 수 있습니다.</p>
        </section>
        <section class="public-notice">
          <h2>방침 변경</h2>
      ...
      "guide.content": `
        <section class="public-section">
          <h2>1. 인원수와 사진 정하기</h2>
          <ol>
            <li>편집기에서 1~5인 구성을 선택합니다. 인원수에 맞춰 카드의 배치와 정보 영역이 바뀝니다.</li>
            <li>이미지 영역의 추가 버튼으로 PNG, JPG, WebP 사진을 한 장 또는 여러 장 불러옵니다.</li>
            <li>사진을 넣은 뒤 카드 미리보기에서 프레임, 전체 보기·채우기, 위치와 크기를 조정합니다. 배경 제거를 사용해도 원본 복원이 가능합니다.</li>
          </ol>
        </section>
        <section class="public-section">
          <h2>2. 장비 정보 추가하기</h2>
          <ol>
            <li>장비 탭에서 이름이나 ID를 검색합니다. 검색 결과에는 가능한 경우 실제 아이템 이미지가 표시됩니다.</li>
            <li>아이템을 추가한 뒤 목록에서 변경하거나 제거할 수 있습니다. 3인 이상 카드의 장비 정보 표시 여부도 스타일 탭에서 조절합니다.</li>
            <li>장비 정보는 카드 설정에 따라 표시되며, 모든 편집 패널의 문구가 카드에 자동으로 인쇄되는 것은 아닙니다.</li>
          </ol>
        </section>
        <section class="public-section">
          <h2>3. 카드 꾸미기</h2>
          <ul>
            <li>제목과 설명을 입력하고 카드 기준으로 왼쪽·가운데·오른쪽 정렬을 선택합니다.</li>
            <li>색상은 배경 명도에 따라 읽기 쉬운 색을 자동으로 선택하거나 직접 지정할 수 있습니다. 이미지 실루엣 위 글자도 대비를 고려해 조절됩니다.</li>
            <li>서체, 굵기, 테두리, 그림자, 배경·패턴·질감을 조합합니다. 작은 화면에서는 컨트롤을 펼쳐 필요한 설정만 확인할 수 있습니다.</li>
          </ul>
        </section>
        <section class="public-section">
          <h2>4. 미리보기와 저장 확인</h2>
          <p>내보내기 버튼을 누르면 현재 카드의 배치, 이미지 위치, 제목 스타일을 기준으로 PNG를 만듭니다. 저장 파일은 화면의 미리보기와 같은 카드 모델을 사용하므로, 변경 직후에는 최신 미리보기가 반영되었는지 확인한 뒤 다시 내보내세요.</p>
          <p>다운로드가 막히면 브라우저의 다운로드 권한, 이미지 형식, 저장 공간을 확인하고 페이지를 새로고침하기 전에 다시 시도해 주세요.</p>
        </section>
        <section class="public-grid">
          <div class="public-section">
            <h2>배경 제거가 처음일 때</h2>
            <p>브라우저 모델 준비에 시간이 걸릴 수 있습니다. 원본은 유지한 채 결과만 바꾸며, 처리가 끝나기 전에는 실루엣 선택이 제한될 수 있습니다.</p>
          </div>
          <div class="public-section">
            <h2>언어 변경</h2>
            <p>상단 언어 선택은 장비 이름만이 아니라 페이지 안내, 편집기 UI와 접근성 라벨에도 적용됩니다. 처음 방문하면 브라우저 언어와 지역을 참고해 기본 언어를 정하고, 이후 선택은 이 브라우저에 기억합니다.</p>
          </div>
        </section>
        <section class="public-notice">
          <h2>도움이 더 필요하면</h2>
      ...
      "contact.content": `
        ...
        <section class="public-section">
          ...
          <p>사진 원본, 계정 비밀번호, 민감한 개인정보는 문제 해결에 꼭 필요할 때만 보내고, 필요하지 않다면 첨부하지 마세요.</p>
        </section>
        <section class="public-notice">
          <h2>삭제·권리 요청</h2>
      ...
      "supportChooser.note": "결제수단과 통화는 선택한 서비스의 결제 화면에서 확인할 수 있습니다.",
      "support.content": `
        <section class="public-notice">
          <h2>후원 안내</h2>
          <p>후원은 선택 사항이며 편집기 기능을 사용하기 위한 결제가 아닙니다. 후원 여부에 따라 기능, 공개 범위, 문의 처리 순서가 달라지지 않습니다. 모든 후원은 일회성으로 안내하며, 세금공제나 기부금영수증 발급을 보장하지 않습니다.</p>
        </section>
        <section class="support-callout">
          <h2>후원 서비스 선택</h2>
          <p>Buy Me a Coffee와 Ko-fi 중 편한 서비스를 선택해 주세요. 원하는 서비스를 고르면 해당 결제 페이지가 새 탭에서 열립니다.</p>
          <button class="support-action" id="supportChooserButton" type="button">후원하기</button>
          <p class="support-status">두 서비스 모두 일회성 후원이며, 후원 여부에 따라 기능이 달라지지 않습니다.</p>
        </section>
        <section class="public-section">
          <h2>결제와 개인정보</h2>
          <p>후원 결제는 외부 결제 제공자가 처리합니다. 결제 카드 정보는 이 사이트가 직접 받지 않습니다. 결제 완료, 환불, 영수증과 결제 정보 처리에 관한 문의는 결제 제공자의 안내를 먼저 확인해 주세요.</p>
          <p>후원 링크가 작동하지 않거나 결제 관련 도움이 필요하면 <a class="public-inline-link" href="../contact/">문의 페이지</a>의 이메일로 알려 주세요.</p>
        </section>`,
    },
    en: {
