...
공통 편집 규칙 후속 분리: models/card-png.js가 확정된 편집 데이터·장비 문구·디코딩된 사진으로 PNG를 만들고, models/editor-navigation.js·title-typography.js가 키보드·서체 규칙을 공유한다. 배경은 app.js의 단색·패턴·질감 직접 선택만 사용한다. app.js는 DOM 조작·서체 준비·제목 측정·편집 변경 감지·다운로드 UI를 담당한다. 5개 구성에서 변경 전후 실제 PNG 픽셀 해시 일치를 확인했다. 최신 로컬 검수에서는 패턴·질감·폰트 레일, 어두운 카드 가독성, 이미지 배치 도크 포커스까지 확인했으며, CSS 전면 통합과 실제 기기 검수는 남아 있다.
...
카드 편집기 핵심과 아이템 검색은 동작 검증을 마쳤고, 지금은 **5단계 배포 준비의 로컬 QA 게이트**까지 진행됐다. GitHub 원격 저장소와 Cloudflare Pages Production 배포는 연결되어 있으며, 최신 원격 기준은 `e095263`이다. 운영 GPU 엔드포인트와 네트워크가 허용된 브라우저의 실제 모델 추론 품질은 아직 확인하지 않았다.
...
| 4. 시각 품질 | 구현·검증 완료 | 지정 폰트 10종, UI·본문 Pretendard, 이름 기반 굵기 선택, 반응형 제목 맞춤, 패턴 레시피, 카드와 PNG 레이어 순서 통일, 모바일 라인업 정보 계층, 패턴·질감 썸네일 시각 QA | 폰트별 실제 원본 조합 QA |
| 5. 접근성·성능·배포 | 로컬 QA 게이트 완료 | Pages 아이템 Function, GPU 배경제거 프록시, 8-buffer WebGPU 게이트와 WASM 폴백, 자동 회귀·대비·AX 트리 검사, 이미지 배치 도크 초기 포커스, 빈 운영 데이터 경계와 전체 로컬 삭제 | 실제 모바일·스크린리더 수동 점검, 네트워크 허용 브라우저의 모델 품질, 운영 GPU 배포·부하 테스트 |
...
- 이미지 배치 도크를 열 때 선택된 사진 대신 닫기 컨트롤에 초기 포커스를 두고, 도크 안에서 방향키·확대·축소 단축키를 계속 사용할 수 있게 했다.
- BiRefNet 1024 WebGPU 모델의 공식 8-buffer 계약에 맞춰 어댑터 게이트를 조정했다. 모델 생성 또는 추론이 실패하면 512 WebGPU, 이후 512 WASM으로 내려가며 회귀 검사는 16-buffer 어댑터에서 이 경로를 확인한다.
- 배경 팔레트·대비 잉크·레거시 초안 변환·재질 자산 선택을 `models/background-style.js`의 순수 정책 모듈로 모으고, 로컬 서버 공개 목록과 모델 회귀 검사를 함께 갱신했다.
- 자동 저장의 디바운스·최신 스냅샷 재실행·실패 상태·전체 삭제 취소 경합을 `models/draft-autosave.js`로 분리하고, 저장 조정기 모델 회귀 검사를 추가했다.
- 저장 룩의 기본값·폰트·색상·장비 목록·에디터 필드 정규화와 런타임 이미지 URL 제거를 `models/look-record.js`로 모아 로드·저장 경계를 하나의 인터페이스로 통일했다.

## 다음 순서

1. 실제 모바일 브라우저와 VoiceOver/NVDA에서 현재 로컬 QA 결과를 수동 확인한다.
2. Preview 배포에서 아이템 검색과 배경 제거 미설정 상태를 확인한다.
3. 네트워크가 허용된 브라우저에서 BiRefNet WebGPU·WASM 추론과 FF14 원본 조합 품질을 골든셋으로 확인한다.
4. 관리형 HTTPS GPU 서비스에 `scripts/background_service.py` 계약을 배포하고 `CUTOUT_SERVICE_URL`, `CUTOUT_SERVICE_TOKEN`을 Preview부터 연결한다.
5. 실제 GPU 결과와 FF14 골든셋을 확인한 뒤 HR/SAM 보정 기능의 범위를 결정하고, 그 다음 CSS·app.js 추가 분리를 진행한다.
