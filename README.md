...
Cloudflare Pages 배포 전에는 한국어 인덱스를 한 번 생성합니다. 네트워크가 끊겼거나 레코드가 0개인 경우 빌드를 실패시키므로 임의 데이터가 운영 데이터로 들어가지 않습니다. 운영 첫 화면은 빈 LOOK BOOK로 시작하며 이미지·장비는 사용자가 추가한 뒤에만 저장됩니다.
