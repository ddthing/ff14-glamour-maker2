...
await page.locator('[data-floating-align="center"]').focus();await page.keyboard.press('ArrowLeft');assert.equal(await page.locator(':focus').getAttribute('data-floating-align'),'left');console.log('floating alignment focus preserved');
const chooser=page.waitForEvent('filechooser');await page.locator('#portraitWrap .character-figure.is-empty').focus();await page.keyboard.press('Enter');await chooser;console.log('preview upload keyboard OK');
